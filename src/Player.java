...
public class Player extends Vec2 {
    ...
    public void draw(Graphics g) {
        ...
        // Draw Rays
        int rayColor = 255;
        int mx, my, mp, dof; double ra=0, xo=0, yo=0, rx=0, ry=0, distH, distY, dist=0;
        ...
        for (int r = 0; r<60; r++) {
            ...
            while(dof<8) {
                ...
                if(mp>0 && mp<mapX*mapY && Map.map[mp]==1) { hv.x=rx; hv.y=ry; dof=8; } // hit wall
            ...
            while(dof<8) {
                mx = (int)(rx)>>6; my = (int)(ry)>>6; mp=my*mapX+mx;
                if(mp>0 && mp<mapX*mapY && Map.map[mp]==1) { yv.x=rx; yv.y=ry; dof=8; } // hit wall
                else { rx+=xo; ry+=yo; dof+=1; } // next line
            }
            ...
            if (distY < distH) {v=yv; dist=distY; g.setColor(new Color(240, 0, 0));} // Vertical Wall Hit
            if (distH < distY) {v=hv; dist=distH; g.setColor(new Color(190, 0, 0));} // Horizontal Wall hit

            g.drawLine((int)(x), (int)(y), (int) (v.x), (int) (v.y)); 
            // g.setColor(new Color(200,200,255)); g.drawOval((int)(v.x), (int)(v.y), 3, 3);
